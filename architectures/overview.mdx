...
The 15 values that resolve to a real architecture:
...
<Warning>
`"auto"` is present in the `SwarmType` type hint but has **no registered factory**. It passes construction and then raises `ValueError` the first time you call `run()`. `"AutoSwarmBuilder"` is not a valid `SwarmType` at all and fails at construction. To have the framework choose for you, instantiate [`AutoSwarmBuilder`](/api/auto-swarm-builder) directly instead of routing through `SwarmRouter`.
</Warning>
