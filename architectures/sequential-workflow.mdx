...
<Note>
  `shared_memory_system` is currently stored on the workflow instance but is not wired into the underlying `AgentRearrange`/agent execution — passing it does not yet change runtime behavior. Reach for `persistent_memory` on individual agents if you need working memory today.
</Note>
