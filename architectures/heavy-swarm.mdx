...
- Optional Grok 4.20 Heavy modes (4-agent and 16-agent rosters)
...
The default Research/Analysis/Alternatives/Verification roster can be replaced with one of two named-agent architectures modelled on X.AI's Grok heavy implementation. Set exactly one of `use_grok_agents` or `use_grok_heavy` — they are mutually exclusive, and enabling both raises a `ValueError`.

### Four-agent Grok mode (`use_grok_agents=True`)

A Captain Swarm orchestrator coordinates three focused specialists:

| Agent | Role |
|-------|------|
| Captain Swarm | Leader and orchestrator |
| Harper | Research and facts |
| Benjamin | Logic, math, and code |
| Lucas | Creative and divergent thinking |
...
### 16-agent Grok Heavy mode (`use_grok_heavy=True`)

A Grok captain decomposes the task into 15 domain-specific questions. All 15 specialists run in parallel, then the captain synthesizes their outputs into a single response.
...
<ParamField path="use_grok_agents" type="bool" default="False">
  Run the four-agent Grok 4.20 Heavy architecture (Captain Swarm + Harper, Benjamin, Lucas). Mutually exclusive with `use_grok_heavy`
</ParamField>

<ParamField path="use_grok_heavy" type="bool" default="False">
  Run the 16-agent Grok 4.20 Heavy architecture (Grok captain + 15 domain specialists). Mutually exclusive with `use_grok_agents`
</ParamField>
...
# - use_grok_agents and use_grok_heavy are not both True
