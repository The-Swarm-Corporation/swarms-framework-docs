...
The `medium` variant is inspired by the [Grok 4.20 Heavy architecture](https://x.com/elonmusk/status/2034710771075273177) (X.AI's multi-agent system) and decomposes work by **thinking style** — each agent applies a fundamentally different cognitive approach.
...
The `heavy` variant mirrors the [Grok 4.20 Heavy 16-agent system](https://x.com/xai/status/2024121909286494435) and decomposes work by **domain expertise** — each agent is a specialist in a distinct field.
...
Same as `get_questions_only()`, but returns the questions as an ordered list. The order is fixed per variant, matching the agent tables above:

- **`default`** (4 items) — research, analysis, alternatives, verification
- **`medium`** (3 items) — Harper, Benjamin, Lucas
- **`heavy`** (15 items) — Harper, Benjamin, Lucas, Olivia, James, Charlotte, Henry, Mia, William, Sebastian, Jack, Owen, Luna, Elizabeth, Noah

On error, returns a single-item list containing the error message.
...
Create and return the variant's agents as a dictionary of named `Agent` instances (called automatically during initialization). All keys are lowercase:

- **`default`** — `research`, `analysis`, `alternatives`, `verification`, `synthesis`
- **`medium`** — `captain` (agent named *Captain-Swarm*), `harper`, `benjamin`, `lucas`
- **`heavy`** — `captain` (agent named *Grok*), plus one first-name key per specialist: `harper`, `benjamin`, `lucas`, `olivia`, `james`, `charlotte`, `henry`, `mia`, `william`, `sebastian`, `jack`, `owen`, `luna`, `elizabeth`, `noah`

Note the coordinator's key is always `captain`, even though the agent's display name differs by variant (*Captain-Swarm* in `medium`, *Grok* in `heavy`).

```python
agents = swarm.create_agents()
print(agents["research"].agent_name)

heavy = HeavySwarm(variant="heavy")
print(len(heavy.create_agents()))            # 16
print(heavy.create_agents()["captain"].agent_name)  # Grok
```
...
2. Loop 2+: `"Previous loop results: <output>\n\nOriginal task: <task>\n\nBased on the previous results and analysis, continue with the next iteration. Refine, improve, or complete any remaining aspects of the analysis. Build upon the insights from the previous loop to provide deeper analysis."`
