...
- 16 swarm types supported, plus `"auto"` (17 `SwarmType` values total — see the note below on `"auto"` / `"AutoSwarmBuilder"`)
...
<Warning>
  `"auto"` and `"AutoSwarmBuilder"` are valid `SwarmType` strings — they pass `reliability_check()` at construction time — but the router's internal factory table (`_swarm_factory`) currently has **no registered factory** for either one. Calling `router.run()` with `swarm_type="auto"` or `swarm_type="AutoSwarmBuilder"` raises a `ValueError` ("Invalid swarm type ...") at run time. If you want the framework to auto-configure agents and swarm choice for you, instantiate `AutoSwarmBuilder` directly (`from swarms import AutoSwarmBuilder`) instead of going through `SwarmRouter`.
</Warning>
