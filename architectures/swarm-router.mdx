...
- 15 swarm types supported through a single interface
...
<Note>
  `"auto"` and `"AutoSwarmBuilder"` are also defined in the `SwarmType` Literal, but the current factory does not dispatch them — selecting either passes construction validation and then fails at run time. Use one of the 15 values above.
</Note>
