...
- 15 concrete swarm types, plus `"auto"` (16 `SwarmType` values total — see the note below on `"auto"`)
...
"RoundRobin"
"PlannerWorkerSwarm"
"auto"             # accepted by validation, but see warning below
```

<Warning>
  `"AutoSwarmBuilder"` is **not** a valid `SwarmType` at all — passing it raises `SwarmRouterConfigError` at construction, not at `run()`. `"auto"` is a valid Literal value but has no registered factory, so it fails when the swarm is built. If you want the framework to auto-configure agents and swarm choice for you, instantiate `AutoSwarmBuilder` directly (`from swarms import AutoSwarmBuilder`) instead of going through `SwarmRouter`.
</Warning>
...
<Warning>
`shared_memory_system` and `telemetry_enabled` **do not exist** on `SwarmRouter`. Because `__init__` accepts `**kwargs`, passing them is silently ignored rather than raising, so code following the old docs appears to work while wiring up nothing. There is no `activate_shared_memory()` method either.
</Warning>

<ParamField path="shared_memory_system" type="Any">
  **Does not exist in code.** See the warning below.
...
`shared_memory_system` accepts any object and is assigned directly to every agent's `long_term_memory` attribute (`activate_shared_memory()` in `swarm_router.py`); the router does not ship a built-in backend. Supply your own memory object that satisfies whatever interface your agents expect:

```python
memory = MyMemoryBackend()  # any object exposing the interface your agents expect
...
<Note>
  `swarms.memory` / `ChromaDB` no longer exist in this version of the framework. For built-in persistence, prefer each agent's own `persistent_memory=True` (writes/reads `MEMORY.md` per agent) instead of a router-level shared backend.
</Note>
...
self._swarm_factory = {
    ...
    # ... one entry per factory-backed SwarmType ...
    # Note: "auto" and "AutoSwarmBuilder" have no entry here — see the
    # warning under "Supported Swarm Types" above.
}
