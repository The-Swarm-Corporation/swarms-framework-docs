...
<ParamField path="director_model_name" type="str" default='"gpt-5.4"'>
  Model used by the auto-created director agent.
</ParamField>

<ParamField path="director_settings" type="Dict[str, Any]">
  Extra `Agent` keyword arguments for the auto-created director. Recognised keys are
  `agent_name`, `model_name`, `system_prompt`, `temperature`, and `top_p`; each one
  overrides the corresponding `HierarchicalSwarm` default (including
  `director_model_name`).
</ParamField>
...
<Note>
  Both parameters are forwarded only when `swarm_type="HierarchicalSwarm"`; other swarm
  types ignore them.
</Note>
