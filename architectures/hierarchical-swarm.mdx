...
Orders for multiple agents run in parallel using a `ThreadPoolExecutor` (with `max_workers = max(1, int(os.cpu_count() * 0.75))`):
...
When `parallel_execution=True`, `execute_orders` submits all of a loop's orders to a `ThreadPoolExecutor` (sized to `max(1, int(os.cpu_count() * 0.75))` workers) so they run concurrently, then writes the results back to the conversation in submission order.
