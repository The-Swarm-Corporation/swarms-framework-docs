...
If `--task` is omitted, the CLI creates the agent and prints its configuration without running it — it does not drop you into a REPL. Use `swarms chat` for an interactive session.
...
- `init` - Scaffold a new project with .env, workspace directory, and validation
...
- `models` - List, search, and inspect LLM models available via LiteLLM
- `tips` - Show rotating tips and tricks for the CLI
...
- `XAI_API_KEY` - xAI / Grok API key (offered by `swarms init`)
- `OPENROUTER_API_KEY` - OpenRouter API key (offered by `swarms init`)
