...
That's it. The CLI builds an Agent with autonomous looping enabled (`max_loops="auto"`) and drops you into an interactive REPL. Responses print once each is complete — `swarms chat` does not enable streaming or autosave by default. Type `exit` or hit `Ctrl-C` to leave the session.
...
swarm_architecture:
  ...
  description: Research a topic, then write a brief from the research
...
The CLI ships with 83 categorized tips covering every command and flag. The banner rotates one per invocation, but you can also pull them on demand:
...
`--saved-state-path` is a write target only — nothing in the CLI or the `Agent` class loads it back on a later run, so pointing a new invocation at the same path does not resume the earlier session.
