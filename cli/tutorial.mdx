...
| `--autosave --saved-state-path <path>` | Save a JSON state snapshot for review |
...
## 5. Run repeatable CLI agents

Use a repeatable project layout when an agent needs files, memory, or external tools. This keeps outputs easy to find and makes later runs easier to debug.
...
export AGENT_STATE_DIR="$(pwd)/agent-state"
```

`WORKSPACE_DIR` is the directory agents use for local files and persistent `MEMORY.md` files. Put source material, drafts, exports, and generated reports there. Keep state snapshots in a separate `agent-state` directory so you can delete working files without deleting audit snapshots.

If you prefer `.env`, store `WORKSPACE_DIR` and API keys there:
...
Use an absolute path for `AGENT_STATE_DIR`. An absolute path prevents the saved JSON snapshot from being nested under a generated agent workspace.

Run `setup-check` before longer jobs:

```bash
swarms setup-check --verbose
```

This catches missing API keys, missing workspace directories, and dependency problems before the model starts spending tokens.

For a one-shot task, run `swarms agent` with a task and a named state snapshot path:

```bash
swarms agent \
  --name "IncidentResearcher" \
  --description "Reads workspace files and writes concise incident notes" \
  --system-prompt "You are a careful incident researcher. Read and write files only under WORKSPACE_DIR." \
  --task "Read the logs in WORKSPACE_DIR and write a short incident brief." \
  --model-name "claude-sonnet-4-6" \
  --max-loops 3 \
  --streaming-on \
  --verbose \
  --autosave \
  --saved-state-path "${AGENT_STATE_DIR}/incident-researcher.json"
```

Use `swarms chat` when you want a conversation. Use `swarms agent --task` when you want a scriptable job that starts, completes, and returns control to the shell.

After the run, inspect outputs, persistent memory, and state snapshots:

```bash
find "$WORKSPACE_DIR" -maxdepth 3 -type f
find "$AGENT_STATE_DIR" -maxdepth 2 -type f
```

The CLI writes persistent conversation memory under a stable agent-name path:

```bash
cat "$WORKSPACE_DIR/agents/IncidentResearcher/MEMORY.md"
```

Use one agent name per role. A research agent, a code reviewer, and a report writer should each get their own name so their `MEMORY.md` files stay separate. Shared memory can leak irrelevant context into later tasks and make results harder to reproduce.

Treat `--saved-state-path` as a state snapshot for auditing and debugging. The CLI does not expose a separate load-state flag. If you need to load a JSON state file directly, use the Python API and call `Agent.load(path)`.

Add MCP tools only after the base run works:

```bash
swarms agent \
  --name "ToolResearcher" \
  --description "Uses approved MCP tools to inspect project data" \
  --system-prompt "Use MCP tools when they help. Keep outputs under WORKSPACE_DIR." \
  --task "Summarize the current project status from the available tool data." \
  --model-name "claude-sonnet-4-6" \
  --mcp-url "http://localhost:8000/sse" \
  --autosave \
  --saved-state-path "${AGENT_STATE_DIR}/tool-researcher.json" \
  --verbose
```

Treat MCP servers as part of the agent's execution boundary. Connect trusted servers, prefer read-only tools for analysis, and keep credentials out of ad-hoc local tool servers. When an MCP run fails, retry with `--verbose`, confirm the server URL, and test the server independently before changing the prompt.

This layout also works well in `tmux`, cron jobs, and CI. Keep the command in a shell script, commit the prompt and expected output path, and leave generated files under `WORKSPACE_DIR` for review.

---

## 6. Discover Models: `swarms models`
...
## 7. Auto-Generate a Swarm: `swarms autoswarm`
...
## 8. Heavy Analysis: `swarms heavy-swarm`
...
## 9. Multi-Model Debate: `swarms llm-council`
...
**Save a state snapshot for review:**

```bash
swarms agent ... --autosave --saved-state-path "${AGENT_STATE_DIR}/research.json"
# Persistent CLI memory lives under $WORKSPACE_DIR/agents/<agent-name>/MEMORY.md
```
