...
Create and run a custom agent with specified parameters. Use `swarms agent` with `--task` for scriptable one-shot work. Use `swarms chat` for a conversational terminal session.
...
- `--interactive` - Allow interactive-capable agent behavior
- `--no-interactive` - Disable interactive-capable agent behavior
...
For a conversational session, use `swarms chat`:
```bash
swarms chat \
  --name "Assistant" \
