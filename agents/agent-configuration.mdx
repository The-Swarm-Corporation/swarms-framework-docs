...
<ParamField path="context_length" type="int" default="None">
  ...
  <Note>
    In the current release, the constructor argument is accepted but the internal `self.context_length` is unconditionally set to `16000` after initialization, regardless of the value passed in. Passing a custom `context_length` currently has no effect on the agent's behavior — this is a known limitation, not documented API behavior.
  </Note>
...
<ParamField path="saved_state_path" type="str" default="Auto-generated">
  ...
  <Note>
    In the current release, this constructor argument is accepted but `self.saved_state_path` is unconditionally overwritten during initialization with an auto-generated `{api_key}_state.json` filename. A custom `saved_state_path` passed at construction time currently has no effect — this is a known limitation, not documented API behavior.
  </Note>
...
Location in source: `swarms/structs/agent.py:348-438` (the `Agent.__init__` signature)
