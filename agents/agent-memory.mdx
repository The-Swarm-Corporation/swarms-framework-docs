...
Swarms agents have disk-backed persistent memory through the `Conversation` class. Each agent writes its active interaction log to a `MEMORY.md` file and reloads that file when another process starts an agent with the same `agent_name`.
...
<Note>
  Persistent memory is keyed by `agent_name`. Reusing the same `agent_name` resumes the same memory across process restarts. Changing the name starts a separate memory folder.
</Note>
...
If an agent should keep only in-process memory, set `memory_md_path` to `None`:
...
This does not disable `conversation_history`. It only stops future writes to the disk-backed `MEMORY.md` file.
