...
<ParamField path="long_term_memory" type="Union[Callable, Any]" default="None">
  Vector database (or callable) attached to the agent for long-term storage.

  <Note>
    In the current release, `long_term_memory` is persisted via `.save()` when `autosave` writes agent state to disk, but `agent.run()` does not automatically query it for retrieval-augmented context — there is no built-in `rag_every_loop` or `memory_chunk_size` parameter, and no automatic `.query()` call in the run loop. If you need RAG-style retrieval inside the loop, wire the vector database up as a [tool](/agents/agent-tools) instead of relying on `long_term_memory` alone.
  </Note>
</ParamField>
