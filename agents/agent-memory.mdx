...
<ParamField path="agent_name" type="str" default="Generated">
  ...
  <Warning>
    Set this explicitly when using `persistent_memory`. An omitted name is
    generated fresh on every construction, so a restarted agent gets a new
    folder and will not find its previous `MEMORY.md`.
  </Warning>
...
<ParamField path="context_compression" type="bool" default="True">
  ...
  agent = Agent(
      ...
      context_length=32000,  # required — compression measures against this
  )
  ```
</ParamField>
...
<Warning>
  `context_length` defaults to `None`. With no `context_length`, the compressor measures a usage ratio of `0.0` and never fires, even with `context_compression=True`. Set `context_length` on any long-running agent that relies on compression.
</Warning>
