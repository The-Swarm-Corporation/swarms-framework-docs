...
Swarms agents persist their interaction history to disk through a per-agent `MEMORY.md` file. Reusing the same `agent_name` across process starts resumes the same memory.
...
Set `agent_name` and run the agent. The memory file is created automatically:
...
agent.run("Research low-latency cloud data warehouses for analytics.")
...
Re-instantiate an agent with the **same** `agent_name`. The prior transcript is preloaded as a system message before the new task runs:
...
agent = Agent(
    agent_name="ResearchAgent",  # same name -> same memory
    model_name="claude-sonnet-4-6",
    max_loops=1,
)
...
For privacy-sensitive or one-off agents, disable disk writes by clearing `memory_md_path`. In-process `conversation_history` still works:
...
agent.run("This task is not written to MEMORY.md.")
```
