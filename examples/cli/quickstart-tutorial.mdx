...
- `gpt-5.4` for the researcher (good cost/quality tradeoff)
- `gpt-5.4` for the reviewer
...
swarm_architecture:
  name: Research-Brief-Swarm
  description: Research a topic, draft a briefing, then review it
...
Each agent receives the previous one's output as context. The Researcher's notes flow into the Writer's brief, which flows into the Reviewer's critique. Total cost on `gpt-5.4` + `claude-opus-4-7` will be a few cents.
...
swarm_architecture:
  name: Perspectives-Swarm
  description: Three independent takes on the same question
...
swarms heavy-swarm \
  ...
  --question-agent-model-name "gpt-5.4" \
  --verbose
```
...
`--saved-state-path` is a write target only: `--autosave` writes agent state to `./helios.json`, but nothing in the CLI or the `Agent` class loads that file back on a later run. Pointing a new `swarms agent` invocation at the same path will not resume the earlier session or its memory.
...
The agent's LLM call fails against the invalid model, and the CLI's error classifier catches it and prints a hint pointing you to `swarms models --search <name>` to find a valid model. (The "Could not load info" message with closest-match suggestions is specific to `swarms models --info <name>`, not the `agent` command.)
