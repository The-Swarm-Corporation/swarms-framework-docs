---
title: 'Multi-Agent CLI Quickstart'
description: 'Spin up a multi-agent system from the Swarms CLI in under a minute.'
icon: 'rocket'
---
...
swarms heavy-swarm \
    ...
    --loops-per-agent 2 \
    --question-agent-model-name gpt-5.4-mini \
    --worker-model-name gpt-5.4-mini \
    --verbose
```
...
| `--question-agent-model-name` | gpt-5.4 | Model for question agent |
| `--worker-model-name` | gpt-5.4 | Model for worker agents |
...
swarms agent \
    --name "Research-Agent" \
    --description "Summarizes recent developments in a field" \
    --system-prompt "You are a research analyst who summarizes recent developments." \
...
Display all available CLI commands and flags:
...
- Explore [CLI Reference Documentation](/cli/commands) for all commands
- See [CLI Examples](/cli/overview) for more use cases
- Learn about [LLM Council](/api/llm-council) Python API
- Try [Heavy Swarm Documentation](/api/heavy-swarm) for advanced configuration
