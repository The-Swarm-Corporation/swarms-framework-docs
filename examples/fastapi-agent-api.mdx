...
def create_agent(agent_name: str = "default", max_loops: int = 1) -> Agent:
    return Agent(
...
async def run_agent(request: AgentRequest):
    try:
        ...
        # max_loops only takes effect at Agent construction time, not on
        # agent.run(), so the requested value must be passed to create_agent().
        agent = create_agent(request.agent_name, request.max_loops)
...
<Note>
  Source: [examples/guides/deployment/fastapi/fastapi_agent_api_example.py](https://github.com/kyegomez/swarms/blob/master/examples/guides/deployment/fastapi/fastapi_agent_api_example.py)
</Note>
