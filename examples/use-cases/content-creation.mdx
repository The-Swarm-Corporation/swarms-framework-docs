...
content_pipeline = SequentialWorkflow(
    ...
    max_loops=1,
    output_type="final",  # return the final draft as a string
)
...
writer_agent = Agent(
    ...
    stopping_condition=lambda response: "content meets quality standards" in response.lower(),
)
```
...
- Explore [Swarm Architectures](/concepts/swarms) for alternative pipeline designs
- Review [Agent Memory](/agents/agent-memory) for maintaining brand consistency
