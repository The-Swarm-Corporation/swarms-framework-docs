...
| Feature | Description |
|---------|-------------|
| **Function-calling schema** | The builder is given `build_agents` as a tool with `tool_choice="auto"` — the model is *not* forced to call it, so a malformed or missing call raises a `ValueError` rather than being silently retried |
...
task
  │
  ▼
builder agent ──build_agents() tool call──▶ roster
                                               │
                          ┌────────────────────┼────────────────────┐
                          ▼                    ▼                    ▼
