---
title: 'Job Finding Swarm'
description: 'Use a swarm to discover, filter, and apply to jobs that match a candidate profile.'
icon: 'briefcase'
---
...
| **David-Search-Executor**   | Runs job searches           | Uses `get_jobs` (RapidAPI's JSearch API) for each query; analyzes and categorizes job results by match strength. |
...
1.  **Install dependencies:**
    ...
    ```
2.  **Set up API Keys:**
    The `David-Search-Executor` agent's `get_jobs` tool calls the JSearch API on RapidAPI, which requires a RapidAPI key sent as the `x-rapidapi-key` header. Subscribe to the [JSearch API](https://rapidapi.com/letscrape-6bRBa3QguO5/api/jsearch) on RapidAPI to get a key.
