...
`docker compose build` has no `-t` flag — set the tag via the `image:` key in `docker-compose.yaml` (replace `<your-dockerhub-username>` there with your actual DockerHub username) instead:

```bash
# Build the image tagged by docker-compose.yaml's `image:` key
...
services:
  swarms-agent-server:
    image: <your-dockerhub-username>/swarm-agent-node:latest
    build: .
...
<Note>
  Replace `<your-dockerhub-username>` with your actual DockerHub username when running the commands above.
</Note>
