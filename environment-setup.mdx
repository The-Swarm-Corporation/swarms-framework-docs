...
<ParamField path="OPENAI_API_KEY" type="string" required>
  Your OpenAI API key for using GPT models (gpt-5.4, gpt-5.4-mini, etc.)
...
<Note>
  The workspace directory can only be set via the `WORKSPACE_DIR` environment variable. `Agent` has no `workspace_dir` constructor parameter — a value passed there is ignored, and the agent always reads its workspace location from the environment.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Model Providers" icon="microchip" href="/integrations/model-providers">
    Learn about all supported LLM providers and how to use them
  </Card>
</CardGroup>

<Info>
  For more detailed information about environment configuration, visit the [official documentation](/environment-setup).
</Info>