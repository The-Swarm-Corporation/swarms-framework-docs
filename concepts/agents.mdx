...
- **Persistent memory**: An opt-in `MEMORY.md` file that survives process restarts
...
Give an agent access to external documents or a database with a retrieval tool:

```python
KNOWLEDGE = {
    "storage": "Grid-scale batteries cover 4-hour peaks at $95/kWh.",
...
def search_knowledge_base(query: str) -> str:
    ...
    """
    hits = [t for k, t in KNOWLEDGE.items() if k in query.lower()]
...
Swap the dictionary for a vector store, SQL query, or HTTP call — the shape stays the same.
...
Separately, `swarms.artifacts.Artifact` (`swarms/artifacts/main_artifact.py`) is a standalone utility for versioning and saving file content (PDF, MD, TXT, and more) — the `Agent` class itself does not use it.
