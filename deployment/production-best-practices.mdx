...
from swarms.schemas.agent_errors import (
    ...
    AgentLLMInitializationError,  # LLM initialization failures
...
# Manual compaction: collapse history into a single summary message.
# `agent.short_memory` is the agent's underlying Conversation object.
agent.short_memory.compact(summary="Summary of the conversation so far.")
```
...
agent = Agent(
    ...
    saved_state_path="./agent_states/stateful_agent.json",
)
```
