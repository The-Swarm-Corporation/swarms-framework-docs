...
<Warning>
**`SkillOrchestra` is no longer part of the library.** It was moved out of `swarms.structs` and now ships only as an example. `from swarms import SkillOrchestra` and `from swarms.structs import SkillOrchestra` both raise `ImportError`.

The code lives at [`examples/multi_agent/skill_orchestra_examples/skill_orchestra.py`](https://github.com/kyegomez/swarms/blob/master/examples/multi_agent/skill_orchestra_examples/skill_orchestra.py). To use it, copy that module into your project and import it locally. Everything below describes that module, not a shipped structure.
</Warning>
