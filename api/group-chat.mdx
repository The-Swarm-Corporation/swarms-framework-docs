...
|---|---|---|
| `round_robin_speaker` | `(agents: List[str], current_index: int = 0)` | `str` — next agent name |
| `random_speaker` | `(agents: List[str], **kwargs)` | `str` — randomly chosen agent name |
| `priority_speaker` | `(agents: List[str], priorities: dict, **kwargs)` | `str` — weighted-random agent name |
| `random_dynamic_speaker` | `(agents: List[str], response: str = "", strategy: str = "parallel", **kwargs)` | `str` or `List[str]` — agents from `@mentions` in the last response |
| `expertise_based` | `(history: List[str], agent: Agent)` | `bool` — whether this agent should speak based on system-prompt match |
