...
<ParamField path="context_length" type="int" default="None">
  Effective context window in tokens. When `context_compression=True`, the agent compresses memory once usage crosses 90% of this limit. Note: in the current release the value you pass is not applied — the agent always uses `16000` internally regardless of this argument.
</ParamField>
...
<ParamField path="tools_list_dictionary" type="List[Dict[str, Any]]" default="[]">
...
<ParamField path="workspace_dir" type="str">
  Not a constructor argument — any value passed here is ignored. The workspace directory is always read from the `WORKSPACE_DIR` environment variable (must be set, e.g. via a `.env` file; see `.env.example` for the conventional value `agent_workspace`). Each agent gets its own subdirectory at `workspace_dir/agents/{agent-name}-{uuid}/`.
</ParamField>
...
### receive_message / receieve_message / send_agent_message

`receive_message` formats an incoming message from another agent and runs it through the agent's normal `run()` loop, returning the agent's response. `receieve_message` (note: this exact spelling is the method name in source) instead appends the message directly to short-term memory without invoking the LLM. `send_agent_message` runs a message addressed to another agent through `run()` and returns the result.

```python
def receive_message(agent_name: str, task: str, *args, **kwargs) -> Any
def receieve_message(name: str, message: str) -> None
...
|--------|-------------|---------------|
| `run(task, img, imgs, correct_answer, streaming_callback, n)` | Run the autonomous agent loop | `agent.run("Generate a report")` |
...
| `receieve_message(name, message)` | Append a message directly to memory (no LLM call) | `agent.receieve_message("User", "Hello")` |
...
| `handle_artifacts(text, file_output_path, file_extension)` | Save artifacts from execution | `agent.handle_artifacts("content", "outputs/", ".txt")` |
...
| `think` | Pause and reason about the current state before acting | `current_state` (str), `analysis` (str), `next_actions` (list), `confidence` (float) |
...
The `create_plan`, `think`, `subtask_done`, and `complete_task` tools are the framework's own planning/control-flow tools; the remainder are governed by `selected_tools`.
...
These live in `swarms.schemas.agent_errors` so collaborator classes can raise and catch them without importing `Agent`. They remain importable from `swarms.structs.agent` and `swarms.schemas`, and every path returns the same object:

```python
from swarms.schemas import AgentError, AgentRunError        # canonical home
from swarms.structs.agent import AgentError                 # also works
...
`Agent.run` is instrumented with OpenTelemetry. When `SWARMS_TELEMETRY_ON` is enabled, each run emits an `Agent.run` span carrying the task, the output, status, and the agent's identity — nested under the swarm run that invoked it, if any. Construction emits an `Agent.init` span carrying the full constructor configuration.

Telemetry is off unless explicitly enabled, and costs ~0.14 µs per call when off. See the [Telemetry guide](/deployment/telemetry) for configuration and how to turn it on or off.
...
| `dynamic_context_window` & `tokens_checks()`             | Optimize token usage with the dynamic_context_window parameter and the tokens_checks() method.   |
