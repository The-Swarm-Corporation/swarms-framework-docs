...
<ParamField path="max_tokens" type="int" default="16000">
  Maximum number of tokens in the LLM response.

  <Warning>
  The value you pass is currently overwritten during setup: the agent uses the model's own maximum output-token limit, falling back to `16000`. Passing `max_tokens` has no effect today.
  </Warning>
</ParamField>
