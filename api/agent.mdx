...
<ParamField path="agent_name" type="str" default="Generated">
  The name of the agent, used for identification and logging. Omit it and the
  agent generates a unique name of the form `agent-k3f9q` (five random
  lowercase alphanumerics), fresh per instance.

  <Note>
    This used to default to the literal `"swarm-worker-01"`, so every agent
    that did not set a name shared it. Anything keyed on the name then
    collided — concurrent result dictionaries kept one entry for the whole
    group, and `MEMORY.md` folders were shared. Pass an explicit name whenever
    you need it to be stable across restarts.
  </Note>
</ParamField>
