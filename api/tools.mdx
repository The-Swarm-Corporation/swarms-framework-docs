...
tools = get_mcp_tools_sync(server_path="http://localhost:8000/sse")
```

<ParamField path="server_path" type="str" default="None">
  Path/URL to the MCP server
</ParamField>

<ParamField path="format" type="str" default="openai">
  Format to return tools in (`"openai"` or `"mcp"`)
</ParamField>

<ParamField path="connection" type="MCPConnection" default="None">
  Optional `MCPConnection` object
</ParamField>

<ParamField path="transport" type="str" default="streamable-http">
  Transport type. Auto-detected from `server_path` if `None`
</ParamField>
...
Asynchronously retrieve tools from an MCP server. Accepts the same parameters as `get_mcp_tools_sync` (`transport` defaults to `None`, which auto-detects).

```python
from swarms.tools import aget_mcp_tools

tools = await aget_mcp_tools(server_path="http://localhost:8000/sse")
```

### execute_tool_call_simple

Async function that executes a tool call on an MCP server.

```python
import asyncio
from swarms.tools import execute_tool_call_simple

result = asyncio.run(execute_tool_call_simple(
    response={"name": "my_tool", "arguments": {"arg1": "value1"}},
    server_path="http://localhost:8000/sse",
))
```

<ParamField path="response" type="any" default="None">
  The tool call request (dict or JSON string)
</ParamField>
...
<ParamField path="output_type" type="Literal['json', 'dict', 'str', 'formatted']" default="str">
  Output format type
</ParamField>

<ParamField path="transport" type="str" default="None">
  Transport type. Auto-detected from `server_path` if `None`
</ParamField>

### get_tools_for_multiple_mcp_servers

Retrieve tools from multiple MCP servers concurrently using a thread pool.

```python
from swarms.tools import get_tools_for_multiple_mcp_servers

all_tools = get_tools_for_multiple_mcp_servers(
    urls=["http://localhost:8000/sse", "http://localhost:8001/sse"]
)
```

<ParamField path="urls" type="List[str]" required>
  List of server URLs to fetch tools from
</ParamField>

<ParamField path="connections" type="List[MCPConnection]" default="None">
  Optional list of `MCPConnection` objects
</ParamField>

<ParamField path="format" type="str" default="openai">
  Format to return tools in
</ParamField>

<ParamField path="output_type" type="Literal['json', 'dict', 'str']" default="str">
  Output format type
</ParamField>
...
<ParamField path="transport" type="str" default="None">
  Transport type. Auto-detected per URL if `None`
</ParamField>
