...
description: "Thread-safe task execution engine that manages background sub-agent tasks with status tracking, result aggregation, retry policies, and depth-limited recursion"
icon: "sitemap"
---

The `SubagentRegistry` is a thread-safe task execution engine that manages background sub-agent tasks with status tracking, result aggregation, retry policies, and depth-limited recursion. It is the core runtime behind the [sub-agent delegation](https://github.com/kyegomez/swarms/blob/master/docs/swarms/examples/sub_agent_tutorial.md) feature.
...
|----------|-------------|
| [Sub-Agent Delegation Tutorial](https://github.com/kyegomez/swarms/blob/master/docs/swarms/examples/sub_agent_tutorial.md) | End-to-end guide with examples for using sub-agents through the autonomous loop |
| [Autonomous Agent Tutorial](https://github.com/kyegomez/swarms/blob/master/docs/swarms/examples/autonomous_agent_tutorial.md) | Full guide to autonomous mode (`max_loops="auto"`) |
| [Agent Reference](/api/agent) | Complete `Agent` class API documentation |
| [AgentRegistry](/api/agent-registry) | Separate registry for managing collections of `Agent` instances (not related to async task execution) |
