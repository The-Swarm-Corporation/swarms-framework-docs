...
Builds a `SwarmRouterConfig` for the task via the boss agent, then constructs and runs a `SwarmRouter` with the given agents. Not called automatically by `run()` for any `execution_type` — call it directly if you want `AutoSwarmBuilder` to both design agents (via `create_agents`) and immediately execute them through a `SwarmRouter`.
...
<ParamField path="output" type="dict" required>
