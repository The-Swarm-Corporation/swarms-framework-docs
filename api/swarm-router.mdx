...
### `update_system_prompt_for_agent_in_swarm()` · `list_agents_to_eachother()`

Internal helpers invoked by `setup()`: append the multi-agent collaboration prompt to each agent, and (when `list_all_agents=True`) add the agent roster to the swarm conversation. Rarely called directly.
