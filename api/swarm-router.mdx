...
<Note>
  `"AutoSwarmBuilder"` is **not** in the `SwarmType` Literal — passing it raises `SwarmRouterConfigError` at construction. `"auto"` is in the Literal (16 values total) but has no registered factory. Use one of the 15 concrete types.
</Note>
